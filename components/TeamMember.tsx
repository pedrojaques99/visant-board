...
export const TeamMember = ({ member, index }: TeamMemberProps) => {
  // Check if this is the "You" member
  const isYou = member.name === "Miguel Soares";
  ...
  return (
    ...
    >
      ...
      <div className="p-8 rounded-2xl border border-[#52ddeb]/10 group-hover:border-[#52ddeb]/30 transition-colors duration-500 text-center">
        <div className="relative w-32 h-32 mx-auto mb-6 rounded-full overflow-hidden border-2 border-[#52ddeb]/20 group-hover:border-[#52ddeb]/40 transition-colors duration-300 flex items-center justify-center bg-background">
          {isYou ? (
        ...
        <h3 className="text-2xl font-bold mb-2 group-hover:text-[#52ddeb] transition-colors duration-300">
          {isYou ? "You" : member.name}
        </h3>
        <p className="text-muted-foreground">
          {isYou ? "Send your portfolio" : member.role}
        </p>
      </div>
    </motion.div>
  );
